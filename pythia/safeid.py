...
def check(password, w, t, z, p, server=defaultServer):
    """
    Checks an existing @password against the Pythia server using 
    values (w,t,z,p).
    @returns: True if the password passes authentication; False otherwise.he 	
    """
...
def query(password, w, t, server=defaultServer, previousPubkey=None):
    ...
    p,y,c,u = (vpop.unwrapP(p), vpop.unwrapY(y), vpop.unwrapC(c), vpop.unwrapU(u))
